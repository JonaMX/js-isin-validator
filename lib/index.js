...
const isinRegex    = /^([A-Z]{2})((?![A-Z]{10}\b)[A-Z0-9]{9})([0-9])$/;

const calcIsinCheckDigit = isin => {
  const [code, checkDigit]        = R.splitAt(-1, isin);
  const [countryCode, securityId] = R.splitAt(2, code);
...
const isIsinValid = isin => {
  ...
  if (!R.find(R.propEq('code', countryCode))(countries)) { return false; }
